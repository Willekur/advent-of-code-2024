...
func main() {
	...
	final_value := 0
	for index := range left_values {
		if right_values[index] > left_values[index] {
			final_value += right_values[index] - left_values[index]
		} else {
			final_value += left_values[index] - right_values[index]
		}
	}
